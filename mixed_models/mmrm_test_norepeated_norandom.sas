...
%if "&dsname"="un" %then %do;
  proc mixed data=adlbh;
     where base ne . and visitnum not in (., 99) and visitnum=floor(visitnum);
...
data diffs;
  set diffs;
  format pval_interaction 20.17;
run;

proc sort data=diffs;
  by descending paramcd descending covariatestructure visitnum trtan;
...
proc export data=diffs
   outfile="/lillyce/qa/vct/common/tfl/test_andy/phuse/output/LSMeansDifferences_ANCOVAwithoutRepeatedMeasures_&ddfmvalue._ADLBH_GithubData.csv" 
   dbms=csv replace;
run;

proc export data=lsmeans
   outfile="/lillyce/qa/vct/common/tfl/test_andy/phuse/output/LSMeans_ANCOVAwithoutRepeatedMeasures_&ddfmvalue._ADLBH_GithubData.csv" 
   dbms=csv replace;
run;
